...
<head>
    ...
    <style>
        ...
        .table-text {
            font-size: 0.7rem; /* Adjust this size to match the table text */
                ...
                <div class="container mt-4">
                    <form method="GET" action="">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="program" name="program_id" required>
                                ...
                                $programsResult = $conn->query("SELECT id, program_name, major FROM programs");
                                while ($program = $programsResult->fetch_assoc()) {
                                    $programDisplay = $program['program_name'] . ' - ' . $program['major'];
                                    echo '<option value="' . $program['id'] . '">' . $programDisplay . '</option>';
                                }
                                ?>
                            </select>
                        </div>
                    ...
                    if (isset($_GET['program_id'])) {
                        ...
                        SELECT 
                            ...
                            materials.material  -- Fetch the material type
                        FROM subjects
                        ...
                        LEFT JOIN material_books ON books.id = material_books.book_id  -- Join with material_books
                        LEFT JOIN materials ON material_books.material_id = materials.id  -- Join with materials
                    ...
                    if (!$reportResult) {
                        ...
                        // Initialize total book count and volumes
                        $totalBooks = 0;
                        $totalVolumes = 0; 
                        ...
                        echo '<div>';
                        echo '<h7 class="card-title mb-0">' . $programNameWithMajor . '</h7>';
                        echo '</div>';
                     ...
                     // Check if there are results
                        if ($reportResult->num_rows > 0) {
                            ...
                            while ($row = $reportResult->fetch_assoc()) {
                                ...
                                if ($row['subject_code'] !== $currentSubjectCode) {
                                    ...
                                    if ($currentSubjectCode !== '') {
                                        ...
                                        echo '<h7 class="table-text">Total Books: ' . $totalBooks . '</h7><br>'; // Display total books centered
                                        echo '<h7 class="table-text">Total Volumes: ' . $totalVolumes . '</h7>'; // Display total volumes centered
                                        echo '</div>'; // Close text-center div
                                    ...
                                    echo '<thead><tr>';
                                    echo '<th style="font-weight: normal;">COURSE NUMBER: ' . htmlspecialchars($row['subject_code']) . '</th>';
                                    ...
                                    if ($row['author'] || $row['title']) {
                                        ...
                                        $totalBooks++; // Increment totalBooks for each book displayed
                                        $totalVolumes += intval($row['volumes']); // Increment totalVolumes
                                    } else {
                            ...
                            // After processing the last subject, display totals
                            echo '<div class="text-center mb-3">';
                            echo '<h7 class="table-text">Total Books: ' . $totalBooks . '</h7><br>'; // Display total books centered
                            echo '<h7 class="table-text">Total Volumes: ' . $totalVolumes . '</h7>'; // Display total volumes centered
                            echo '</div>'; // Close text-center div
                        ...
                        echo '<div style="margin-top: 10px; margin-left: 40px;  font-size: 0.7rem;">'; // Added a div to wrap the name input for spacing
                        echo '<input type="text" id="preparedBy" name="prepared_by" placeholder="Enter name here" style="border: none; border-bottom: 1px solid black; width: 200px; outline: none; font-size: 1em;  font-weight: bold;" />'; // Name input field
                        echo '</div>'; // Close div for name input
                        echo '<div style="margin-top: 5px; margin-left: 40px;  font-size: 0.7rem;">'; // Added a div for position input
                        echo '<input type="text" id="position" name="position" placeholder="Position" style="border: none; width: 100%; outline: none; font-size: 1em;" />'; // Position input field
                        echo '</div>'; // Close div for position input
                        echo '</div>'; // Close prepared by section
                        ...
                        echo '</tr>';
                        echo '<tr>';
                        echo '<td style="padding-left: 20px; font-size: 0.6rem;">Effectivity: <input type="date" id="effectivity" name="effectivity" style="border: none; width: 120px; outline: none; font-size: 0.7rem;" />';
                        ...
                        echo '</tr>';
                        echo '<tr>';
                        echo '<td style="padding-left: 20px; font-size: 0.6rem;">Revision: <input type="text" id="revision" name="revision" placeholder="Enter revision" style="border: none; width: 100px; outline: none; font-size: 0.7rem;" /></td>'; // Input for Revision in the third row
                        echo '</tr>';
                    ...
                    <script>
                        document.getElementById("effectivity").addEventListener("change", function() {
                            var dateInput = new Date(this.value);
                            var options = { year: 'numeric', month: 'long', day: 'numeric' };
                            var formattedDate = dateInput.toLocaleDateString('en-US', options);
                            document.getElementById("formatted-effectivity").innerText = formattedDate;
                            document.getElementById("formatted-effectivity-hidden").value = formattedDate; // Store formatted date
                        });
                    </script>
                        ...
                        function printReport(courseName, majorName) {
                            ...
                            var styles = `
                             ...
                             // Include prepared by information
                                var preparedBy = document.getElementById("preparedBy").value || "Name not provided";
                                var position = document.getElementById("position").value || "Position not provided";
                               
                                 // Get formatted date from the display span
                                var effectivity = document.getElementById("formatted-effectivity").innerText || "Effectivity date not provided";
                                var revision = document.getElementById("revision").value || "Revision not provided";


                                // Prepared by section with inline styles
                                var preparedByContent = `
                                    <div class="prepared-by-section" style="margin-left: 20px; margin-top: 20px;">
                                        ...
                                        <div style="margin-top: 10px; margin-left: 40px;"><br>
                                            <input type="text" value="${preparedBy}" style="border: none; border-bottom: 1px solid black; width: 200px; outline: none; font-size: 1em; font-weight: bold;" readonly />
                                        </div>
                                        <div style="margin-top: 5px; margin-left: 40px;">
                                            <input type="text" value="${position}" style="border: none; width: 100%; outline: none; font-size: 1em;" readonly />
                                        </div>
                                    </div>
                              `;
                                ...
                                var footerContent = `
                                    <div class="footer-section">
                                        ...
                                        <div>Effectivity: ${effectivity}</div>
