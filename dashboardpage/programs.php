...
if (isset($_POST['add_program'])) {
    ...
    $programMajors = isset($_POST['majors']) ? $_POST['majors'] : []; // Ensure majors is an array

    // If no majors are selected, add 'None' as a default
    if (empty($programMajors)) {
        $programMajors = ['None'];
    }

    // Replace empty majors with 'None'
    $programMajors = array_map(function($major) {
        return empty($major) ? 'None' : $major;
    }, $programMajors);

    // Check for duplicates before proceeding with the insert
    $duplicateFound = false;
    foreach ($programMajors as $major) {
        $major = mysqli_real_escape_string($conn, $major); // Sanitize input

        if ($major === 'None') {
            $checkDuplicateQuery = "SELECT * FROM `programs` WHERE program_name = '$programName' AND major = 'None'";
        } else {
            $checkDuplicateQuery = "SELECT * FROM `programs` WHERE program_name = '$programName' AND major = '$major'";
        }
        ...
        if (mysqli_num_rows($resultDuplicate) > 0) {
            $duplicateFound = true;
            $errorMessage = "Program '$programName' with major field '$major' already exists.";
            break;
        }
    }

    // If no duplicate found, insert the program(s)
    if (!$duplicateFound) {
        foreach ($programMajors as $major) {
            $major = mysqli_real_escape_string($conn, $major); // Sanitize input

            // Insert program with selected major
            $insertQuery = "INSERT INTO `programs` (`major`, `program_name`, `program_acronym`) VALUES ('$major', '$programName', '$programAcronym')";
            mysqli_query($conn, $insertQuery);
        }

        // Redirect to avoid re-submit on page refresh
...
if (isset($_POST['update'])) {
    ...
    $program_acronym = mysqli_real_escape_string($conn, $program_acronym);
    $major = mysqli_real_escape_string($conn, $major);

    // Check for duplicate program-major combination
    if ($major === 'None') {
        $checkDuplicateQuery = "SELECT * FROM `programs` WHERE program_name = '$program_name' AND major = 'None' AND id != $Id";
    } else {
        $checkDuplicateQuery = "SELECT * FROM `programs` WHERE program_name = '$program_name' AND major = '$major' AND id != $Id";
    }
    ...
    if (mysqli_num_rows($resultDuplicate) > 0) {
        ...
        $errorMessage = "Program '$program_name' with major field '$major' already exists.";
    } else {
...
<body>
                                        ...
                                        <div class="col-lg-4 col-md-3 col-sm-3 col-xs-10 mt-3">
                                            ...
                                            <div id="major-container">
                                                <div class="input-group mb-2">
                                                    <input type="text" name="majors[]" class="form-control form-control-sm" placeholder="Enter Major">
                                                    <button type="button" class="btn btn-sm remove-major">Remove</button>
                                                </div>
                                            </div>
                                            <small class="form-text text-muted mt-2">Leave this field empty if there is no major.</small>
                                            <button type="button" class="btn btn-sm mt-2" id="add-major">add another major</button>
                                        </div>
    ...
    <script>
        document.getElementById('add-major').addEventListener('click', function() {
            const majorContainer = document.getElementById('major-container');
            const majorField = document.createElement('div');
            majorField.classList.add('input-group', 'mb-2');
            majorField.innerHTML = `
            <input type="text" name="majors[]" class="form-control form-control-sm" placeholder="Enter Major">
            <button type="button" class="btn btn-sm remove-major">Remove</button>
        `;
            majorContainer.appendChild(majorField);

            majorField.querySelector('.remove-major').addEventListener('click', function() {
                majorField.remove();
            });
        });

        // Add functionality to existing remove buttons
        document.querySelectorAll('.remove-major').forEach(button => {
